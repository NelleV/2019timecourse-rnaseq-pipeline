...
The manuscript was last compile on 2020-07-21 17:59:44.
...
\begin{center}\includegraphics[width=.5\textwidth,]{reports/manuscript_files/figure-latex/topPvalueGenes-1} \end{center}
...
\begin{center}\includegraphics[width=.5\textwidth,]{reports/manuscript_files/figure-latex/topLogFold-1} \end{center}
...
\begin{center}\includegraphics[width=.5\textwidth,]{reports/manuscript_files/figure-latex/unnamed-chunk-18-1} \end{center}
...
\CommentTok{# }\AlertTok{FIXME}\CommentTok{ This line fails}
\CommentTok{# dataToPlot = rbind(}
\CommentTok{#     assay(de_moanin_model[cluster2Genes[3:6],]), }
\CommentTok{#    "Centroid"=kmeans_clusters$centroids[cluster_of_interest,])}
\CommentTok{# plot_splines_data(de_moanin_model, data=dataToPlot, }
\CommentTok{#     colors=ann_colors$Group, smooth=TRUE, simpleY=FALSE,}
\CommentTok{#    mar=c(1.5,2.5,2,0.1))}
\end{Highlighting}
\end{Shaded}
...
We can also overlay the centroid on the data, after rescaling the data to be between 0-1, like the centroids

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{de_moanin_rescale =}\StringTok{ }\NormalTok{de_moanin_model}
\KeywordTok{assay}\NormalTok{(de_moanin_rescale) =}\StringTok{ }\KeywordTok{rescale_values}\NormalTok{(}\DataTypeTok{data=}\KeywordTok{assay}\NormalTok{(de_moanin_model))}
\KeywordTok{plot_splines_data}\NormalTok{(de_moanin_rescale,  }
...
\begin{center}\includegraphics[width=.5\textwidth,]{reports/manuscript_files/figure-latex/unnamed-chunk-19-1} \end{center}

\textbf{EAP} I added the functionality to plot centroid over data, rather than just fit to data, but I'm not sure it makes much sense to include this. The \texttt{rescale\_values} function is too sensitive to outliers. Would be better to rescale each centroid to scale of gene. If we had a helper function to do that, could easily add that to plot\_splines\_data (made issue on moanin), but probably right now should remove this.
...
We display for each cluster, the scores of the genes assigned to that cluster (Figure \ref{visualizeScores}), as compared to the cutoff value for what that score must be under our filtering procedure. We can do this by rerunning \texttt{splines\_kmeans\_score\_and\_label} and setting the filter \texttt{percentage\_genes\_to\_label=1} (and we can speed up the calculation by giving our previously calculated score via the argument \texttt{previous\_scores})
...
\NormalTok{unfiltered_scores<-}\KeywordTok{splines_kmeans_score_and_label}\NormalTok{(moanin_model,kmeans_clusters,}\DataTypeTok{proportion_genes_to_label=}\DecValTok{1}\NormalTok{, }\DataTypeTok{previous_scores=}\NormalTok{scores_and_labels}\OperatorTok{$}\NormalTok{scores)}
...
Of the four genes we plotted before, 2 of them remained assigned to cluster 2. We can again look at a
...
\CommentTok{# }\AlertTok{FIXME}\CommentTok{ this lines failes.}
\CommentTok{# dataToPlot = rbind(assay(moanin_model[cluster2Score[1:4], ]),}
\CommentTok{#     "Centroid"=kmeans_clusters$centroids[cluster_of_interest, ])}
\CommentTok{# plot_splines_data(moanin_model, data=dataToPlot, }
\CommentTok{#     colors=ann_colors$Group, smooth=TRUE,simpleY =FALSE,}
\CommentTok{#     mar=c(1.5,2.5,2,0.1))}
\end{Highlighting}
\end{Shaded}
...
Now we will create the heatmaps of these genes (1283 genes, Figure \ref{fig:heatmapCluster8}).
...
\CommentTok{# }\AlertTok{FIXME}\CommentTok{ this line doesn't work}
\CommentTok{# aheatmap(}
\CommentTok{#    rescale_values(moanin_model[genes_to_plot,ord])[res$rowInd,],}
\CommentTok{#    Colv=NA,}
\CommentTok{#    Rowv=NA,}
\CommentTok{#    annCol=orderedMeta[, c("Group", "Timepoint")],}
\CommentTok{#    annLegend=TRUE,}
\CommentTok{#    annColors=ann_colors,}
\CommentTok{#    main=paste("Cluster", cluster_to_plot, "(rescaled)"),}
\CommentTok{#    treeheight=0)}
\end{Highlighting}
\end{Shaded}
...
\caption{Consensus matrix of proportion of times samples were in the same cluster over bootstrap samples for k=5 and 20.}\label{fig:bootstrapConsensus-1}
\end{figure}
...
{\centering \includegraphics[width=.5\textwidth,]{reports/manuscript_files/figure-latex/bootstrapConsensus-2} 

}

\caption{Consensus matrix of proportion of times samples were in the same cluster over bootstrap samples for k=5 and 20.}\label{fig:bootstrapConsensus-2}
\end{figure}
...
\midrule
04740 & Olfactory transduction & 8 & 0\\
04080 & Neuroactive ligand-receptor interaction & 10 & 0\\
\bottomrule
\end{tabular}
...
\midrule
GO:0044260 & cellular macromolecule metabolic process & 3762 & 3607 & 3540.21 & 0.00031 & 0.276\\
GO:0043603 & cellular amide metabolic process & 467 & 458 & 439.47 & 0.00036 & 0.276\\
GO:0043161 & proteasome-mediated ubiquitin-dependent ... & 201 & 199 & 189.15 & 0.00042 & 0.276\\
GO:0016071 & mRNA metabolic process & 285 & 280 & 268.20 & 0.00052 & 0.276\\
GO:0070925 & organelle assembly & 427 & 417 & 401.83 & 0.00052 & 0.276\\
\addlinespace
GO:0019058 & viral life cycle & 123 & 123 & 115.75 & 0.00054 & 0.276\\
GO:0018130 & heterocycle biosynthetic process & 1809 & 1733 & 1702.35 & 0.00093 & 0.348\\
GO:0019438 & aromatic compound biosynthetic process & 1810 & 1734 & 1703.29 & 0.00098 & 0.348\\
GO:0042110 & T cell activation & 273 & 268 & 256.91 & 0.00102 & 0.348\\
GO:0006954 & inflammatory response & 410 & 398 & 385.83 & 0.00104 & 0.348\\
\addlinespace
GO:0006163 & purine nucleotide metabolic process & 199 & 198 & 187.27 & 0.00108 & 0.348\\
GO:0051301 & cell division & 300 & 294 & 282.31 & 0.00109 & 0.348\\
GO:0070507 & regulation of microtubule cytoskeleton o... & 110 & 110 & 103.51 & 0.00120 & 0.361\\
GO:0006091 & generation of precursor metabolites and ... & 222 & 220 & 208.91 & 0.00138 & 0.392\\
GO:0006364 & rRNA processing & 106 & 106 & 99.75 & 0.00153 & 0.412\\
\bottomrule
\end{tabular}
...
##  [3] ggfortify_0.4.10            timecoursedata_0.99.0      
##  [5] moanin_0.99.0-901           SummarizedExperiment_1.18.2
