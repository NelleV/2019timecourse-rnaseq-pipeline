...
\NormalTok{cluster2Score =}\StringTok{ }\NormalTok{cluster2Score[}\KeywordTok{order}\NormalTok{(scores_and_labels}\OperatorTok{$}\NormalTok{scores[cluster2Score, }\DecValTok{8}\NormalTok{], }\DataTypeTok{decreasing =} \OtherTok{TRUE}\NormalTok{)]}
\NormalTok{dataToPlot =}\StringTok{ }\KeywordTok{rbind}\NormalTok{(data[cluster2Score[}\DecValTok{1}\OperatorTok{:}\DecValTok{4}\NormalTok{], ],}\StringTok{"Centroid"}\NormalTok{=kmeans_clusters}\OperatorTok{$}\NormalTok{centroids[}\DecValTok{8}\NormalTok{, ])}
\KeywordTok{plot_splines_data}\NormalTok{(dataToPlot, moanin_model,}
    \DataTypeTok{colors=}\NormalTok{ann_colors}\OperatorTok{$}\NormalTok{Group, }\DataTypeTok{smooth=}\OtherTok{TRUE}\NormalTok{,}\DataTypeTok{simpleY =}\OtherTok{FALSE}\NormalTok{,}
    \DataTypeTok{mar=}\KeywordTok{c}\NormalTok{(}\FloatTok{1.5}\NormalTok{,}\FloatTok{2.5}\NormalTok{,}\DecValTok{2}\NormalTok{,}\FloatTok{0.1}\NormalTok{))}
\end{Highlighting}
...
\textbf{Nelle: in my experience, the large the cluster is, the noisier the centroid
estimation is. I've switched to cluster 8, as this is the cluster we focus our
interpretation, and the top scoring genes seem to be to be closely related to
...
\textbf{EAP: \texttt{subsample\_proportion} was set to 1 in the code, but shouldn't it be
0.8? I changed it to 0.8. But it seems like you don't actually run this code,
but read existing runs, so maybe was right there?}

\textbf{Nelle: that was a mistake in the code. I don't actually run subsampling
experiments: only the bootstrap, and it's done in a separate file as it is too
time consuming to run in an Rmd file}
