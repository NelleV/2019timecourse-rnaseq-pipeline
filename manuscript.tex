...
\textbf{EAP: It seems like a problem to have the colors defined in a utils.R. Makes it hard It's not echo'd by markdown and so readers won't know where they come from. (I didn't).}
...
\emph{EAP}: I moved this here, because we use meta a lot in the eda.
...
changing over time) or or on multiple conditions (such as the influenza
...
\textbf{EAP}: this makes it sound like you create a class, but in fact it's just a list. Wouldn't it be helpful to make it a class?

We start by creating the \texttt{moanin\_model} object using the
\texttt{create\_moanin\_model} function. We need to provide two things to the function: a data.frame with the meta data, and the number of degrees of freedom. The
...
formula object used or the basis or the degrees of freedom of the model.
\textbf{EAP:} You make it sound like or, but in fact the object has all of these elements, no?

If no formula is provided, it will default to the following, which provides for a different spline fit for every group (as defined by the \texttt{Group} variable in the \texttt{meta} data.frame):
...
differential expression analysis. Comparison between groups is traditionally done by
defining the group comparisons (called \texttt{contrasts} in linear models) as a linear combination of the coefficients of the model. Comparing groups within each timepoint can create many contrasts, and thus \texttt{moanin} provides functionality to create these contrasts in an automatic way, and then calls \texttt{limma} \citep{ritchie:limma} on the set of contrasts provided. By default, \texttt{moanin} expects RNA-Seq contact counts, and will estimate voom weights.
...
The output is a table of results, where each row corresponds to a gene
and the columns correspond to the p-value (\texttt{pval}), log-fold change (\texttt{lfc})
and adjusted p-value (\texttt{qval}) of the sets of contrasts; the order of the genes in the table is the same
as the input \texttt{data}. Here we show the results for the first timepoint (i.e.~first three columns of the output) and
the first ten genes:
...
We will repeat this, comparing each of the remaining three treatments to the control (``M'')
...
\textbf{EAP: Okay, I'll leave that to you. Most of those tweaked things can just be in a \ldots{} argument and plotting functions that do annoying things are popular. I've at least rewritten it so users can more easily change those features in calls to the function rather than mucking about in the code}
...
mean functions. This is done with the function \texttt{DE\_timecourse}, which takes as similar input that of \texttt{DE\_timepoints}, only now will test the entire mean function (and therefore does not require a step of expanding the contrasts into contrasts for individual timepoints).
...
\textbf{EAP: is there a reason that the qvalues aren't provided with the output like in \texttt{DE\_timepoints}? Doesn't that risk the user not taking that step and working with the unadjusted p-values?}
...
visualize gene time-course data in different conditions. Here, we plot the 10 genes with the smallest
p-values.
...
\item
  ...
  cluster centroids.
\item
  \textbf{Assign scores and labels to all genes}: Our \texttt{splines\_kmeans\_score\_and\_label} function assigns a score to all gene-cluster pairs based on a goodness-of-fit measure of the gene to the cluster centroid. We then ``assign'' each gene to a cluster based on its score.
\end{itemize}

The first three steps are performed internally by the \texttt{splines\_kmeans} function. For now, we will set the number of clusters to be \(8\), though we will return to the question of picking the best number of clusters below.
...
We then use the \texttt{plot\_splines\_data} function to visualize the centroids of each cluster obtained
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cluster2Genes =}\StringTok{ }\KeywordTok{names}\NormalTok{(kmeans_clusters}\OperatorTok{$}\NormalTok{clusters[kmeans_clusters}\OperatorTok{$}\NormalTok{clusters}\OperatorTok{==}\DecValTok{2}\NormalTok{])}
\NormalTok{dataToPlot=}\KeywordTok{rbind}\NormalTok{(y[cluster2Genes[}\DecValTok{3}\OperatorTok{:}\DecValTok{6}\NormalTok{], ],}\StringTok{"Centroid"}\NormalTok{=kmeans_clusters}\OperatorTok{$}\NormalTok{centroids[}\DecValTok{2}\NormalTok{,])}
...
Since we arbitrarily chose a subset of genes based on our filter, we would like all genes to have a chance of assignment to a cluster, as well as a score for whether it is a good assignment. The scoring and label step allows us to assign genes to clusters that were removed during the
...
function. This function calculates the goodness of fit of the gene to the cluster centroid. By default, the function labels only the 50\% of genes with the best scores, with the remaining genes getting \texttt{NA} as their assignment.
...
\NormalTok{cluster2Score =}\StringTok{ }\KeywordTok{names}\NormalTok{(labels[labels}\OperatorTok{==}\DecValTok{2}\NormalTok{])}
\end{Highlighting}
\end{Shaded}

After running \texttt{scores\_and\_labels} we now have 19772 genes that are assigned to a cluster.
...
\NormalTok{cluster2Score=cluster2Score[}\KeywordTok{order}\NormalTok{(scores_and_labels}\OperatorTok{$}\NormalTok{scores[cluster2Score,}\DecValTok{2}\NormalTok{], }\DataTypeTok{decreasing =} \OtherTok{TRUE}\NormalTok{)]}
\NormalTok{dataToPlot=}\KeywordTok{rbind}\NormalTok{(data[cluster2Score[}\DecValTok{1}\OperatorTok{:}\DecValTok{4}\NormalTok{], ],}\StringTok{"Centroid"}\NormalTok{=kmeans_clusters}\OperatorTok{$}\NormalTok{centroids[}\DecValTok{2}\NormalTok{,])}
\KeywordTok{plot_splines_data}\NormalTok{(dataToPlot, moanin_model,}
    \DataTypeTok{colors=}\NormalTok{ann_colors}\OperatorTok{$}\NormalTok{Group, }\DataTypeTok{smooth=}\OtherTok{TRUE}\NormalTok{,}\DataTypeTok{simpleY =}\OtherTok{FALSE}\NormalTok{,}
    \DataTypeTok{mar=}\KeywordTok{c}\NormalTok{(}\FloatTok{1.5}\NormalTok{,}\FloatTok{2.5}\NormalTok{,}\DecValTok{2}\NormalTok{,}\FloatTok{0.1}\NormalTok{))}
\end{Highlighting}
...
\begin{center}\includegraphics{reports/manuscript_files/figure-latex/unnamed-chunk-28-1} \end{center}
...
\begin{center}\includegraphics{reports/manuscript_files/figure-latex/unnamed-chunk-31-1} \end{center}
...
squares; (2) the labels assigned to all genes. \textbf{EAP: I don't understand what you are trying to say in this last sentence. I think there's a typo}

\textbf{EAP: Also, I don't know what you mean by ``clustering experiment'' -- do you mean a clustering of the data with a particular K? Ditto for ``all possible clusters of interest'' -- do you mean all choices of K, so 5,9,11,15 in the below.}

Below we run the clustering for \(k\) equal to \(2-20\). The \texttt{splines\_kmeans} function returns the WCSS for each cluster, which we sum to get the total WCSS.
...
cluster sum of squares (WCSS) as a function of the number of clusters. At some point,
the WCSS will start decreasing more slowly, giving an angle or ``elbow'' in the graph. The number of
...
\textbf{EAP: I've increased this to be k=2-20, because you can't actually interpret these plots without a dense number of k; also you do 2-20 for stability}
...
\textbf{EAP: \texttt{subsample\_proportion} was set to 1 in the code, but shouldn't it be 0.8? I changed it to 0.8. But it seems like you don't actually run this code, but read existing runs, so maybe was right there?}
...
\textbf{EAP: So this is very confusing to understand, because you read the results from a file (\texttt{stability\_5.tsv}), but I don't know what this file contains, nor how they were calculated. And I don't know what a stability matrix is -- you've only talked about a consensus matrix. There should be the code here for a single k, but not evaluated, and say that for convenience we provide the results of running this code as a file for users to play with. I've tried to fix up the text to be what I think it should be}

We will run \textbf{EAP: subsampling???} method on the top 1000 genes \textbf{EAP: what is top??}, and do it \(B=20\) times for each of \(k=2-20\). We show the code below, but because of the time it takes we have evaluate these values separately and provided the results for users to explore more quickly.
...
\textbf{EAP: It's very confusing that the headings here are C1, \ldots{} C20. It makes it seem like cluster 1, \ldots{} cluster 20, when I think (?) it is really 20 resampling results. Would be good to change that to, e.g.~B1,\ldots{},B20 or something.}

For each gene, we have an assignment to a cluster over the 20 resampling runs. For example, for \(k=5\):

\begin{tabular}{l|r|r|r|r|r|r|r|r|r|r|r|r|r|r|r|r|r|r|r|r}
\hline
  & C1 & C2 & C3 & C4 & C5 & C6 & C7 & C8 & C9 & C10 & C11 & C12 & C13 & C14 & C15 & C16 & C17 & C18 & C19 & C20\\
\hline
AA204140 & 5 & 5 & 5 & 5 & 5 & 1 & 3 & 2 & 1 & 3 & 1 & 3 & 4 & 3 & 5 & 4 & 2 & 4 & 1 & 4\\
\hline
AB008911 & 5 & 5 & 1 & 4 & 1 & 2 & 3 & 3 & 2 & 3 & 3 & 3 & 4 & 4 & 5 & 5 & 2 & 4 & 1 & 4\\
\hline
AB010313 & 3 & 3 & 1 & 4 & 1 & 2 & 4 & 3 & 2 & 5 & 3 & 1 & 5 & 4 & 2 & 5 & 3 & 3 & 4 & 2\\
\hline
AB010342 & 5 & 3 & 1 & 5 & 1 & 1 & 3 & 3 & 1 & 3 & 3 & 3 & 4 & 3 & 5 & 4 & 2 & 3 & 4 & 4\\
\hline
AB011473 & 2 & 1 & 2 & 1 & 3 & 3 & 2 & 5 & 5 & 2 & 4 & 4 & 1 & 1 & 1 & 2 & 5 & 1 & 2 & 1\\
\hline
AB099518 & 2 & 1 & 2 & 1 & 3 & 3 & 2 & 5 & 5 & 2 & 4 & 4 & 1 & 1 & 1 & 2 & 5 & 1 & 2 & 1\\
\hline
\end{tabular}

Now we will use the function \texttt{consensus\_matrix} in the \texttt{moanin} package to calculate proportion of times each pair of samples was clustered together across the 20 resampling runs, and plot the heatmap of those consensus matrices for \(k=5\) and \(k=20\).
...
\begin{center}\includegraphics{reports/manuscript_files/figure-latex/unnamed-chunk-36-1} \end{center}
...
\begin{center}\includegraphics{reports/manuscript_files/figure-latex/unnamed-chunk-36-2} \end{center}
...
is correct, the clustering results are stable to bootstrap resampling, as described above. The
...
The model explorer strategy works as follows. For a single choice of \(k\), first perform \(n\) bootstrap
experiments to estimate the cluster centroids, followed by a step of assigning
a label to all data points. Then, choose a similarity
measure between two partitions or clusters \(S(C_1, C_2)\). \textbf{EAP: This is confusing as to whether this is a measure of similarity across different bootstrap resamples (i.e.~different sets of clusters) or across two different clusters in the same resample. Perhaps also because of confusion of headings of different resamples between labeled C1, C2, but those aren't cluster 1/2 (see comment above)} Examples are the
normalized mutual information or Fowlkes-Mallows. Finally, compute the pairwise similarity measure
between all bootstrapped partitions (i.e.~B choose 2 pairs). Repeat this procedure for different \(k\) and plot per \(k\) the cumulative density of the
obtained scores.
...
The function \texttt{plot\_model\_explorer}
...
\begin{center}\includegraphics{reports/manuscript_files/figure-latex/unnamed-chunk-39-1} \end{center}
...
of elements of the consensus matrix. We provide a function \texttt{plot\_cdf\_consensus} in \texttt{moanin} to do this
...
\textbf{EAP: We should again provide the code that goes with these images}
...
\midrule
GO:0003002 & regionalization & 172 & 153 & 134.41 & 0.00026 & 0.263\\
GO:0048562 & embryonic organ morphogenesis & 152 & 135 & 118.78 & 0.00045 & 0.332\\
GO:0010970 & transport along microtubule & 82 & 75 & 64.08 & 0.00046 & 0.332\\
GO:0006805 & xenobiotic metabolic process & 49 & 47 & 38.29 & 0.00059 & 0.373\\
GO:0060173 & limb development & 92 & 84 & 71.89 & 0.00068 & 0.382\\
\addlinespace
GO:0019395 & fatty acid oxidation & 53 & 51 & 41.42 & 0.00134 & 0.619\\
GO:0055114 & oxidation-reduction process & 536 & 461 & 418.84 & 0.00139 & 0.619\\
GO:0060070 & canonical Wnt signaling pathway & 149 & 131 & 116.43 & 0.00147 & 0.619\\
GO:0090596 & sensory organ morphogenesis & 134 & 118 & 104.71 & 0.00219 & 0.852\\
GO:0060411 & cardiac septum morphogenesis & 33 & 32 & 25.79 & 0.00295 & 1.000\\
\addlinespace
GO:0010811 & positive regulation of cell-substrate ad... & 81 & 73 & 63.30 & 0.00384 & 1.000\\
GO:0042738 & exogenous drug catabolic process & 22 & 22 & 17.19 & 0.00437 & 1.000\\
GO:1901381 & positive regulation of potassium ion tra... & 22 & 22 & 17.19 & 0.00437 & 1.000\\
GO:0007368 & determination of left/right symmetry & 66 & 60 & 51.57 & 0.00536 & 1.000\\
GO:0060078 & regulation of postsynaptic membrane pote... & 66 & 60 & 51.57 & 0.00536 & 1.000\\
\bottomrule
\end{tabular}
