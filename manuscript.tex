...
\textbf{EAP: General comment. Your function names are incredibly long compared to
standard functions. Also, will want to get rid of \texttt{moanin::} in front of
functions. It's more precise, but an average user doesn't ever experience the
\texttt{::} command; usually just package creaters.}
...
Given the source and a \texttt{Targets.txt} structured as follows, microarray can be
...
\item
  ...
  approach first step is to fit a spline model to each gene \textbf{FIXME add refs},
...
time-course datasets, where there are only a few time points \textbf{FIXME add
citation to edgeR/limma vignette or DESEQ2}. For long time-course datasets,
...
the function works with any form contrasts. \textbf{FIXME: Reference to limma
vignette for how to define the contrasts} \textbf{EAP: Could you make simple
function that creates them automatically for two conditions -- i.e.~find union
of time points for two conditions\ldots{} Then we could show an example with just a
couple done manually, and then say we can also use a little helper function in
the \texttt{moanin} that will do it for all time points}
...
\NormalTok{weekly_de_analysis =}\StringTok{  }\NormalTok{moanin}\OperatorTok{::}\KeywordTok{DE_timepoints}\NormalTok{(}
...
\NormalTok{pvalues =}\StringTok{ }\NormalTok{moanin}\OperatorTok{::}\KeywordTok{DE_timecourse}\NormalTok{(}
\NormalTok{    data, splines_model, contrasts,}
    \DataTypeTok{use_voom_weights=}\OtherTok{FALSE}\NormalTok{)}
