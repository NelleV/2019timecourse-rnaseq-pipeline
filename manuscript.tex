...
The per-time point analysis is often the entire approach advocated for dealing with small time-course datasets, where there are only a few time points \textbf{FIXME add citation to edgeR/limma vignette or DESEQ2}. For long time-course datasets, however, such an approach results in creating many different tests, for example for every pair of adjacent timepoints, the results of which are difficult to integrate. We find in practice that the global analysis simplifies analysis and of longer time courses data, with per-time point analysis reserved for particularly interesting comparisons of individual time-points.
...
\textbf{Use with \texttt{moanin}} \texttt{moanin} provides functionality for performing both of these types of approaches. In both situations, we first set up a object (a \texttt{splines\_model} object) to hold the meta data, as well as information for fitting the spline model.
