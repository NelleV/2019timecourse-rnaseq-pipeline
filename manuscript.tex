...
The manuscript was last compile on 2020-06-17 05:37:22.
...
\textbf{EAP}: Why is this list not the same as the (hidden via echo=FALSE) list at the beginning of the document? I also needed to install \texttt{BiocWorkflowTools}, \texttt{KEGGprofile} to run the .Rmd
...
normalization and quality control checks of the data. However, in what follows, we do not show the steps for normalization, as these are specific to the platform (microarray); the code for the normalization can be found in as a supplemental file. \textbf{EAP}: perhaps a citation to review papers for normalization on microarray and mRNA-Seq.
...
\textbf{EAP}: I think it's better here to actually show the output, rather than a table, so you can see what the contrasts actually look like. I like to reserve table format for actual results, rather than code-like input
...
\textbf{EAP:} We shouldn't use any hidden functions (i.e.~that use \texttt{:::} as in \texttt{moanin:::rescale\_values}). We should make it a exported function if use it in workflow.
...
\textbf{EAP} This didn't make sense to me: ``run the clustering for all possible clusters of interest.'' Do you mean across different values of \(k\)? That's what I changed it to.
...
\textbf{EAP}: Why don't you use the same rule/code as above? i.e.~with the fisher p-values etc.? A little strange to have two different filtering rules for the clustering. Or is this the same, and you're restating the rule? (Unclear why you'd have to redo everything here\ldots{}didn't go back and compare code)
...
\textbf{EAP} It's somewhat unfortunately that B=20 and the maximum k=20. Makes it easy to get them confused. Could we switch one of these to be 15 or 25 so as to keep them straight for the reader?
...
\textbf{EAP} I'm confused by this plot. Isn't the numbering here for different values of \(k\)? Why do the labels say ``B1'' and ``B2''? Isn't that the notation above for the bootstrap replications for each values of \(k\)? It seems like the names of \texttt{all\_labels} should be something like \texttt{k=1},\ldots{}, while the names of \texttt{all\_labels{[}{[}1{]}{]}} should be \texttt{B1,B2,...}. But currently they have the same values.
...
Once the gene ID to GO mapping list is created, \texttt{moanin} provides an
interface to \texttt{topGO} to determine enriched GO terms. \textbf{EAP: What does the package do that is an enhancement to topGO output? Would be good to briefly describe} Here, we show an example of
