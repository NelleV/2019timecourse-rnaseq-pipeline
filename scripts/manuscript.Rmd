...
dataToPlot=rbind(assay(de_moanin_model[cluster2Genes[3:6],]), 
...
plot_splines_data(de_moanin_rescale,  data=kmeans_clusters$centroids[cluster_of_interest,], 
    colors=ann_colors$Group, smooth=TRUE,simpleY =FALSE,
    data_smooth_only=TRUE, subset_data=cluster2Genes[3:6],
    mar=c(1.5,2.5,2,0.1))
```
...
We display for each cluster, the scores of the genes assigned to that cluster (Figure \ref{visualizeScores}), as compared to the cutoff value for what that score must be under our filtering procedure. We can do this by rerunning `splines_kmeans_score_and_label` and setting the filter `percentage_genes_to_label=1` (and we can speed up the calculation by giving our previously calculated score via the argument `previous_scores`)
...
orderedMeta<-colData(moanin_model)
ord = order(
  submeta$Group,
  submeta$Timepoint,
  submeta$Replicate)
...
res = aheatmap(
    assay(moanin_model[genes_to_plot,ord]),
    Colv=NA,
    color="YlGnBu",
    annCol=submeta[,
                c("Group", "Timepoint")],
...
n_genes = dim(de_moanin_model)[1] * sample_proportion
indices = sample(1:dim(de_moanin_model)[1], n_genes, replace=TRUE)
...
kmeans_clusters = splines_kmeans_predict(y, kmeans_clusters, method="distance")
```
