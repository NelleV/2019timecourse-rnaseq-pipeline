...
EAP:
- Do we need installation instructions for moanin, since its not on CRAN/bioconductor? Also, how its set up now (doing make file, which compiles the documentation, etc), does that prohibit use of, e.g. `install_github` from `devtools`? I.e. not something we can demonstrate in the markdown file. 
