...
approach first step is to fit a spline model to each gene **FIXME add refs**,
...
time-course datasets, where there are only a few time points **FIXME add
...
the function works with any form contrasts. **FIXME: Reference to limma
vignette for how to define the contrasts** **EAP: Could you make simple
function that creates them automatically for two conditions -- i.e. find union
of time points for two conditions... Then we could show an example with just a
couple done manually, and then say we can also use a little helper function in
the `moanin` that will do it for all time points**
