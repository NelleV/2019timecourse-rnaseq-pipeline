...
**EAP: General comment. Your function names are incredibly long compared to standard functions. Also, will want to get rid of `moanin::` in front of functions. It's more precise, but an average user doesn't ever experience the `::` command; usually just package creaters. **
...
first quality control step. **EAP: Cut or move this sentence, so that this entire section is applicable to microarray or mRNA-Seq**
...
# Copy the metadata before changing the timepoints into factors
data_corr_meta$Timepoint = as.factor(meta$Timepoint)
NMF::aheatmap(
    data_corr,
    Colv=NA, Rowv=NA,
...
`splines_model` object using the `create_splines_model` function.
...
`moanin` provides a simple interface to perform a timepoint by timepoint differential
expression analysis. This is traditionally done by the user defining the comparisons (called `contrasts` in linear models).  Under the hood, simply calls `limma` [@ritchie:limma] on the set of
contrasts provided. **EAP: Can we comment on using it for RNA-Seq??** 
...
(H5N1) ("VL") for each time point, but the function works with any form contrasts. **FIXME: Reference to limma vignette for how to define the contrasts** **EAP: Could you make simple function that creates them automatically for two conditions -- i.e. find union of time points for two conditions... Then we could show an example with just a couple done manually, and then say we can also use a little helper function in the `moanin` that will do it for all time points**

**FIXME rename the function to something better** 

**EAP: And don't use 'week' in function name!**

**Add a helper function to make the contrasts for timely DE**
...
To leverage this temporal structure, Storey et al [@storey:significance] proposed to
