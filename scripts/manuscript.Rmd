...
**EAP:** We shouldn't use any hidden functions (i.e. that use `:::` as in `moanin:::rescale_values`). We should make it a exported function if use it in workflow.  

**EAPL** actually, why do we need to rescale them between 0-1 rather than just a normal call to `scale`? Would it really make a difference?
