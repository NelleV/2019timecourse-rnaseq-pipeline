...
cluster2Score = cluster2Score[order(scores_and_labels$scores[cluster2Score, 8], decreasing = TRUE)]
dataToPlot = rbind(data[cluster2Score[1:4], ],"Centroid"=kmeans_clusters$centroids[8, ])
plot_splines_data(dataToPlot, moanin_model,
    colors=ann_colors$Group, smooth=TRUE,simpleY =FALSE,
    mar=c(1.5,2.5,2,0.1))
```
...
**Nelle: in my experience, the large the cluster is, the noisier the centroid
...
**EAP: `subsample_proportion` was set to 1 in the code, but shouldn't it be
...
**Nelle: that was a mistake in the code. I don't actually run subsampling
