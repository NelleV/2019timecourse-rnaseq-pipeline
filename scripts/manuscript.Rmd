...
- Why `dependicies=FALSE`? (same for the README for the repos). Seems easier to just say dependencies=TRUE and avoid the install.R routine. 
...
- `biocLite` is no longer the recommended way to install bioconductor. Instead
  `BiocManager::install(c("edge", "NMI"))`. But this is dependent on
  bioconductor/R version. Also, now that docker, can you change the
  installation to just be simple one-liner (and `BiocManager::install` can
...
across time by measuring mRNA levels from samples collected at different timepoints [^1]. Such time-course studies can vary from measuring a few distinct time
...
a new variety of time course studies have come from single-cell sequencing experiments**EAP: need citation** which can sequencing single cells which are at different stages of development; in this case, the time point is the stage of the cell in the process of development -- a
...
**EAP: The colors for timepoint appear to circle around, i.e. the first and last are very similar colors**
...
legend(x=250,y=25,legend=names(ann_colors$Group), fill=ann_colors$Group,bty="n",xpd=NA,yjust=0.5)
...
legend(x=250,y=-15,legend=names(ann_colors$Timepoint), fill=ann_colors$Timepoint,bty="n",xpd=NA,yjust=0.5)
par(mar=c(4.5, 5.5, 1.5, .1))
plot(
    pca_data$x[, c("PC3","PC2")],
    col=ann_colors$Timepoint[as.factor(meta$Timepoint)],
...
# Define contrast	
VL_weekly_de_analysis = DE_timepoints(
...
This results in a table of the results, where each row corresponds to a gene and the columns correspond to the p-value (`pval`), log-fold change (`lfc`) and adjusted p-value (`qval`); the order of the genes in the table is the same as the input `data`. Here we show the results for the first two timepoints and the first ten genes: 
...
We will repeat this, comparing each of the treatments to the control ("M") (code not printed here, as it is a replicate of the above, see accompanying Rmarkdown document). 
...
timely_barplot = function(de_analysis, labels=NULL, qval_threshold=0.05, title="",...){
    qval_colnames = colnames(de_analysis)[
	grepl("qval", colnames(de_analysis))]
    if(is.null(labels)) labels<-sapply(strsplit(gsub("-qval","",qval_colnames),"\\."),.subset2,3)
    number_de_genes_per_time = colSums(de_analysis[, qval_colnames] < 0.05)
    barplot(number_de_genes_per_time, col="black", 
            names.arg=labels,ylim=c(0, 20000),
	    xlab="Timepoint", ylab="Number of DE genes", main=title,...)
    
}

par(mfrow=c(2, 2))
timely_barplot(K_weekly_de_analysis, time, title="Kawasaki",las=3)
timely_barplot(C_weekly_de_analysis, time, title="California",las=3)
timely_barplot(VL_weekly_de_analysis, time, title="Vietnam low-dose",las=3)
timely_barplot(VH_weekly_de_analysis, time, title="Vietnam high-dose",las=3)
```
...
Another difficulty with such an approach is making sense of the general temporal structure for any particular gene, as different genes will have different combinations of timepoints DE. For the comparison of the Kawasaki strain to the control, for example, there are `length(signifCombos)` genes found DE in some timepoints, and there are `length(tabCombos)` different combinations of timepoints for which they are DE. Some of these make sense, such as DE in timepoints 48H-168H (`tabCombos["48,60,72,120,168"]` genes), but many are very fragmentary. For example there are `tabCombos["48,60,120,168"]` genes which are DE in timepoints 48,60,120,168H, but *not* in the 72H. Many of these genes are likely to have not made the cutoff for significance in 72H, but don't show real differences in the overall trend between 48H-168H. 
...
wh<-which(splines_model$meta$Group %in% c("K","M"))
plot_genes(data[exampleGenes,],splines_model,colors=ann_colors$Group)
``` 

**EAP: It would be nice to be able to subset in `plot_centroids/plot_genes` to only some of the groups**
