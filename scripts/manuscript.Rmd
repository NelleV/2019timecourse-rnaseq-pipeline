...
knitr::opts_chunk$set(
  ...
  duplicate.label="allow",out.extra = "")
...
In the following plot, we plot the two first PC components in the first column, where each sample is colored by its
condition (top row) and its sampling time (bottom row). 
We use different plotting symbols for each
replicate. We also plot the second and third components in the second column.

```{r pca_plots, echo=FALSE, results="hide", fig.width=6, fig.height=6, fig.cap="Plot of PC components: PC2 vs PC1 (right) and PC2 vs PC3 (left). Samples are colored by condition (top row) and sampling time (bottom row)."}
par(mfrow=c(2, 2))
...
Then, we plot the pearson correlation between each sample as a heatmap diagram (using the function `aheatmap` in `NMF`). We order the
...
```{r  correlation_plot, echo=FALSE, results="hide",fig.width=6, fig.height=6, fig.cap="Heatmap of the correlation between samples"}
