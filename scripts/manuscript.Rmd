...
**EAP: This needs to be worked on, because too big to print. Really, need a `print.moanin_model` in the package (submitted issue) and put that here.**
...
contrasts[1:5]
```
**EAP**: I think it's better here to actually show the output, rather than a table, so you can see what the contrasts actually look like. I like to reserve table format for actual results, rather than code-like input
...
function (and unlike `DE_timecourse` therefore does not require the extra step of expanding the contrasts into contrasts for individual timepoints). 
...
```{r echo=FALSE, fig.cap="Distribution of DE genes per condition based on global splines analysis"}
...
```{r visualizeScores, fig.height=6, fix.width=6, echo=TRUE, fig.cap="Distribution of goodness-of-fit scores for each cluster. The dashed red line indicates the scoring threshold: all genes with a score above this threshold will not be labeled by \\texttt{splines_kmeans_score_and_label}."}
# Get the best score and best label for all of the genes
# This is different from output of `splines_kmeans_score_and_label`, which filters
...
**EAP** This didn't make sense to me: "run the clustering for all possible clusters of interest." Do you mean across different values of $k$? That's what I changed it to.
...
```{r elbow_method, fig.width=6, fig.height=4, fig.cap="Plot of WCSS as a function of k"}
...
```{r average_silhouette_method, fig.width=6, fig.height=4, fig.cap="Plot of silhouette values as a function of k"}
...
**EAP**: Why don't you use the same rule/code as above? i.e. with the fisher p-values etc.? A little strange to have two different filtering rules for the clustering. Or is this the same, and you're restating the rule? (Unclear why you'd have to redo everything here...didn't go back and compare code)
...
**EAP** It's somewhat unfortunately that B=20 and the maximum k=20. Makes it easy to get them confused. Could we switch one of these to be 15 or 25 so as to keep them straight for the reader?
...
```
**EAP** I'm confused by this plot. Isn't the numbering here for different values of $k$? Why do the labels say "B1" and "B2"? Isn't that the notation above for the bootstrap replications for each values of $k$? It seems like the names of `all_labels` should be something like `k=1`,..., while the names of `all_labels[[1]]` should be `B1,B2,...`. But currently they have the same values.
...
Once the gene ID to GO mapping list is created, `moanin` provides an 
interface to `topGO` to determine enriched GO terms. **EAP: What does the package do that is an enhancement to topGO output? Would be good to briefly describe** Here, we show an example of
