...
abstract: >
    ...
    dataset; (2) differential expression analysis using functional
...
BiocWorkflowTools::f1000_article: default  for F1000Research layout
ms_document: default for word document
...
library(topGO)
library(moanin)
if(!packageVersion("moanin")>="0.99.0-902") stop("Must have version 0.99.0-902 to compile the manuscript")
```
...
`moanin` and `timecoursedata` are available from bioconductor, and can be
installed using the `install` function in the package `BiocManager`, along
...
samples in scatter plots. We also reorder the factor `Group` which describes
the treatments so that the treatments are ordered from low to high pathogeny
(with Control being first). 
...
supplemental file [@abrams:protocol; @park:evaluation].  
...
In both situations, we first need to set up an object (a `Moanin` object) to
hold the meta data, as well as information for fitting the spline model
(formula, number of degrees of freedom of the splines, ...). The `Moanin`
...
We start by creating the `Moanin` object using the `create_moanin_model`
...
two columns: one named `Group`, containing the  treatment effect, and a
...
```{r echo=FALSE}
pander(weekly_de_analysis[1:10, 1:3 ])
...
perWeek_barplot(K_weekly_de_analysis, labels=time, main="Kawasaki", las=3)
perWeek_barplot(C_weekly_de_analysis, labels=time, main="California", las=3)
perWeek_barplot(weekly_de_analysis, labels=time, main="Vietnam low-dose", las=3)
perWeek_barplot(VH_weekly_de_analysis, labels=time, main="Vietnam high-dose", las=3)
```
...
```{r showMissingDEGenes, echo=FALSE, fig.width=10, fig.height=12, fig.cap="The expression values over time for 10 genes found significant DE in timepoints 48,60,120,168H, but not in the 72H timepoint."}
...
arguments the data, the `Moanin` object, the contrasts to evaluate, and the method to use to estimate the log-fold change.
...
pander(log_fold_change_timecourse[1:5, ])
``` 
...
```{r topPvalueGenes, fig.height=6, fig.width=6, fig.cap="Top 10 genes with the smallest p-values"}
...
```{r topLogFold,fig.height=6, fig.width=6, fig.cap="Top 10 genes with largest absolute timecourse log-fold change"}
...
After filtering, we are left with `r dim(de_moanin_model)[1]` genes. We can then apply a
...
- **Splines estimation**: for each gene, fit the splines function with the basis
  of your choice (as contained in the `Moanin` object).
...
```{r centroids, fig.height=6, fig.width=6, fig.cap="K-means centroids"}
...
```{r genesVsCentroids, fig.height=6, fig.width=6, fig.cap="Genes from cluster 2"}
...
**Assigning cluster labels:** We could just assign each gene to a cluster
based on which cluster gave the minimum score. By default,
`splines_kmeans_score_and_label` does not do that, but rather requires a
...
all genes on all clusters (i.e. the entire `scores` matrix returned by
`splines_kmeans_score_and_label`). A gene is then assigned to a cluster only
if their best score is above the 50% percentile of that distribution, with the
remaining genes getting `NA` as their assignment (this choice can be changed
by `proportion_genes_to_label`). Note that because the distribution of scores
of genes across *all* clusters is used, this is not equivalent to assigning
...
filtering cutoff of `splines_kmeans_score_and_label`, i.e. if we simply assign
...
`splines_kmeans_score_and_label` and setting the filter
`percentage_genes_to_label=1` (and we can speed up the calculation by giving
our previously calculated score via the argument `previous_scores`)

```{r visualizeScores, fig.height=6, fig.width=6, echo=TRUE, fig.cap="Distribution of goodness-of-fit scores for each cluster. The dashed red line indicates the scoring threshold: all genes with a score above this threshold will not be labeled."}
...
genes_in_cluster2)` of them remained assigned to cluster `r cluster_of_interest` after our scoring. We can again look at a
...
```{r saving_results, results="hide", echo=FALSE}
...
### Looking at specific clusters in detail.
...
```{r heatmapCluster8, fig.width=6, fig.height=8, fig.cap="Heatmap of genes in Cluster 6"}
...
Most methods to find the number of clusters with stability measures
...
same size as our original clustering (i.e. `r dim(de_moanin_model)[1]` genes):
...
we have an assignment to a cluster over the 25 resampling runs. For example,
for the first 10 bootstrap samples and $k=5$:

```{r,echo=FALSE}
pander(head(stability_5[, 1:10]))
```
...
can thus help assess an adequate number of clusters.
...
the packages `biomaRt` [@durinck:biomart] and `KEGGprofile`
[@zhao:keggprofile] for this step. `KEGGprofile` is a package that facilitates
enrichment analysis on a set of genes labeled with the Ensembl annotation
[@ensembl2020] based on the set of biological pathways described in the KEGG
database [@KEGG].

We thus need to convert the gene names, which are given in the Refseq
annotation, into the corresponding Ensembl name. This is where `biomaRt` comes
in handy: it enables easy conversion from one gene annotation to another.
Here, we will use the function `getBM` in `biomaRt` to convert the gene names
from cluster 8.
...
Then we use the function `find_enriched_pathway` in the `KEGGprofile` package
...
pander(pathways[1:min(10,nrow(pathways)), ])
```
...
The Gene Ontology (GO) database [@GO], also categorizes genes into meaningful
biological ontologies and can be used for enrichment analysis via the package
`topGO` [@alexa:topgo]. We again use `biomaRt` to find the mapping between
...
expression data in R using the package `moanin`, which aids in implementing
...
`moanin` allows for easy implementation of these procedures in the setting of
time-course data.
...
Archived source code at the time of publication can be found at [https://osf.io/2a5tw/](https://osf.io/2a5tw/).
...
Normalized data can be found in `timecoursedata`.  Normalization information is
