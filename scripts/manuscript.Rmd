...
## Installation and setup
 ...
 `moanin` is available from GitHub, and can be installed using the `install_github` function in the package `devtools`, along with the corresponding package that contains time course datasets we will use:

```{r eval=FALSE}
library(devtools)
install_github("NelleV/moanin")
install_github("NelleV/timecoursedata")
```
...
```{r eval=FALSE}
BiocManager::install(c("NMF","ggfortify","topGO","biomaRt","KEGGprofile","BiocWorkflowTools"))
```
...
In both
situations, we first need to set up a object (a `Moanin` object) to
hold the meta data, as well as information for fitting the spline model
(formula, number of degrees of freedom of the splines, ...). The `Moanin` object
...
```{r fig.height=6, fig.width=6, fig.cap="K-means centroids"}
...
dataToPlot = rbind(
    assay(de_moanin_model[cluster2Genes[3:6],]), 
    "Centroid"=kmeans_clusters$centroids[cluster_of_interest,])
plot_splines_data(de_moanin_model, data=dataToPlot, 
...
```{r}
de_moanin_rescale = de_moanin_model
assay(de_moanin_rescale) = rescale_values(data=assay(de_moanin_model))
plot_splines_data(de_moanin_rescale,  
    centroid=kmeans_clusters$centroids[cluster_of_interest,], 
    colors=ann_colors$Group, smooth=TRUE,simpleY =FALSE,
    subset_data=cluster2Genes[3:6],
    mar=c(1.5,2.5,2,0.1))
```
**EAP** I added the functionality to plot centroid over data, rather than just fit to data, but I'm not sure it makes much sense to include this. The `rescale_values` function is too sensitive to outliers. Would be better to rescale each centroid to scale of gene. If we had a helper function to do that, could easily add that to plot_splines_data (made issue on moanin), but probably right now should remove this. 
...
of genes across *all* clusters are used, this is not equivalent to assigning
...
unfiltered_scores<-splines_kmeans_score_and_label(moanin_model,kmeans_clusters,proportion_genes_to_label=1, previous_scores=scores_and_labels$scores)
...
dataToPlot = rbind(
    assay(moanin_model[cluster2Score[1:4], ]),
    "Centroid"=kmeans_clusters$centroids[cluster_of_interest, ])
plot_splines_data(
    moanin_model, data=dataToPlot, 
    colors=ann_colors$Group, smooth=TRUE, simpleY=FALSE,
    mar=c(1.5, 2.5, 2, 0.1))
```
...
indices = sample(1:dim(de_moanin_model)[1], n_genes * subsample_proportion, replace=FALSE)
