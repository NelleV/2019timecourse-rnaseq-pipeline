...
We display for each cluster, the scores of the genes assigned to that cluster (Figure \ref{fig:visualizeScores}), as compared to the cutoff value for what that score must be under our filtering procedure. We can do this by rerunning `splines_kmeans_score_and_label` and setting the filter `percentage_genes_to_label=1` (and we can speed up the calculation by giving our previously calculated score via the argument `previous_scores`)
...
dataToPlot = rbind(assay(moanin_model[cluster2Score[1:4], ]),
    "Centroid"=kmeans_clusters$centroids[cluster_of_interest, ])
plot_splines_data(moanin_model, data=dataToPlot, 
     colors=ann_colors$Group, smooth=TRUE,simpleY =FALSE,
     mar=c(1.5,2.5,2,0.1))
```
