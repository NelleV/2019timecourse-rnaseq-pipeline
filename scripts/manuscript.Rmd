...
2) Global analysis, where we consider the expression pattern globally over time, and consider what genes have either different patterns between conditions or a changing pattern (i.e. non-constant) over time. 
...
In both situations, a common approach first step is to fit a spline model to each gene **FIXME add refs**, and then use that spline model to test for different kinds of differential expression across time. 

`moanin` is set up to facilitate working with these spline models for differential expression. Most of
`moanin`'s functions rely on creating a `splines_model` object to contain the metadata and the splines model to be used. 
