...
**EAP: It seems like a problem to have the colors defined in a utils.R. Makes it hard It's not echo'd by markdown and so readers won't know where they come from. (I didn't).**
...
*EAP*: I moved this here, because we use meta a lot in the eda. 
...
**EAP**: this makes it sound like you create a class, but in fact it's just a list. Wouldn't it be helpful to make it a class?

We start by creating the `moanin_model` object using the
`create_moanin_model` function. We need to provide two things to the function: a data.frame with the meta data, and the number of degrees of freedom. The
metadata data.frame object should contain at least two columns: one named
`Group`, containing the  treatement effect, and a second one named `Timepoint`
...
**EAP:** You make it sound like or, but in fact the object has all of these elements, no?

If no formula is provided, it will default to the following, which provides for a different spline fit for every group (as defined by the `Group` variable in the `meta` data.frame): 
...
and adjusted p-value (`qval`) of the sets of contrasts; the order of the genes in the table is the same
as the input `data`. Here we show the results for the first timepoint (i.e. first three columns of the output) and
the first ten genes: 
...
**EAP: Okay, I'll leave that to you. Most of those tweaked things can just be in a ... argument and plotting functions that do annoying things are popular. I've at least rewritten it so users can more easily change those features in calls to the function rather than mucking about in the code**
...
mean functions. This is done with the function `DE_timecourse`, which takes as similar input that of `DE_timepoints`, only now will test the entire mean function (and therefore does not require a step of expanding the contrasts into contrasts for individual timepoints). 
...
**EAP: is there a reason that the qvalues aren't provided with the output like in `DE_timepoints`? Doesn't that risk the user not taking that step and working with the unadjusted p-values?**
...
| abs_sum  | $\sum_t \lvert lfc(t)\lvert$  | Always positive |
| max  | $\max_t \lvert lfc(t) \lvert$  | Always positive  |
| min  | $\min_t \lvert lfc(t) \lvert$  | Always positive |
...
We then use the `plot_splines_data` function to visualize the centroids of each cluster obtained
...
```{r fig.height=6, fig.width=6}
cluster2Genes = names(kmeans_clusters$clusters[kmeans_clusters$clusters==2])
dataToPlot=rbind(y[cluster2Genes[3:6], ],"Centroid"=kmeans_clusters$centroids[2,])
...
function. This function calculates the goodness of fit of the gene to the cluster centroid. By default, the function labels only the 50% of genes with the best scores, with the remaining genes getting `NA` as their assignment.
...
After running `scores_and_labels` we now have `r length(labels)` genes that are assigned to a cluster. 
...
Of the four genes we plotted before, `r sum(cluster2Genes[3:6] %in% cluster2Score)` of them remained assigned to cluster 2. We can again look at a few genes in cluster 2, only now pick the best scoring genes 

```{r fig.height=6, fig.width=6}
cluster2Score = names(labels[labels==2])
#order them by their score
cluster2Score=cluster2Score[order(scores_and_labels$scores[cluster2Score,2], decreasing = TRUE)]
dataToPlot=rbind(data[cluster2Score[1:4], ],"Centroid"=kmeans_clusters$centroids[2,])
plot_splines_data(dataToPlot, moanin_model,
    colors=ann_colors$Group, smooth=TRUE,simpleY =FALSE,
    mar=c(1.5,2.5,2,0.1))
```
...
**EAP: I don't know what to think about these plots -- these are best scoring, but pretty noisy. Also the per-gene spline is sometimes not as good of a explanation as the cluster centroid. It's seems like an important plot to make**
...
squares; (2) the labels assigned to all genes. **EAP: I don't understand what you are trying to say in this last sentence. I think there's a typo**

**EAP: Also, I don't know what you mean by "clustering experiment" -- do you mean a clustering of the data with a particular K? Ditto for "all possible clusters of interest" -- do you mean all choices of K, so 5,9,11,15 in the below.**
...
**EAP: I've increased this to be k=2-20, because you can't actually interpret these plots without a dense number of k; also you do 2-20 for stability**
...
Using the bootstrapping strategy we sample with replacement a sample of the same size as our original clustering (i.e. `r dim(y)[1]` genes):
...
**EAP: `subsample_proportion` was set to 1 in the code, but shouldn't it be 0.8? I changed it to 0.8. But it seems like you don't actually run this code, but read existing runs, so maybe was right there?**
...
**EAP: So this is very confusing to understand, because you read the results from a file (`stability_5.tsv`), but I don't know what this file contains, nor how they were calculated. And I don't know what a stability matrix is -- you've only talked about a consensus matrix. There should be the code here for a single k, but not evaluated, and say that for convenience we provide the results of running this code as a file for users to play with. I've tried to fix up the text to be what I think it should be**


We will run **EAP: subsampling???** method on the top 1000 genes **EAP: what is top??**, and do it $B=20$ times for each of $k=2-20$. We show the code below, but because of the time it takes we have evaluate these values separately and provided the results for users to explore more quickly. 
...
```
**EAP: It's very confusing that the headings here are C1, ... C20. It makes it seem like cluster 1, ... cluster 20, when I think (?) it is really 20 resampling results. Would be good to change that to, e.g. B1,...,B20 or something.**
...
The function `plot_model_explorer`
...
**EAP: `plot_model_explorer` is relatively slow, because it does all those pairwise calculations. And then to do it for smaller subset, redo all the calculations. Can't the output be saved, or the calculations be a different step from the plotting?**
...
of elements of the consensus matrix. We provide a function `plot_cdf_consensus` in `moanin` to do this
...
```
**EAP: We should again provide the code that goes with these images**
...
Archived source code at the time of publication can be foud at [XXX](XXX).
